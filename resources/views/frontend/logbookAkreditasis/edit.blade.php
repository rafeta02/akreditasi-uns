        ...
        <form method="POST" action="{{ route("admin.logbook-akreditasis.update", [$logbookAkreditasi->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('valid') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="valid" id="valid" value="1" {{ $logbookAkreditasi->valid || old('valid', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('validated_by') ? 'is-invalid' : '' }}" name="validated_by_id" id="validated_by_id">
                    @foreach($validated_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('validated_by_id') ? old('validated_by_id') : $logbookAkreditasi->validated_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
